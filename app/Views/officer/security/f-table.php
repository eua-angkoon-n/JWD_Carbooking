...
Class DataTable extends TableProcessing {
    ...
    public function createArrayDataTable($fetchRow, $numRow){
        ...
        if (count($fetchRow) > 0) {
            ...
            foreach ($fetchRow as $key => $value) {
                $id      = $fetchRow[$key]['id_security'];
                $checked = ($fetchRow[$key]['security_status']==1 ? 'checked value="1" disabled' : ' disabled ');

                $dataRow = array();
                $dataRow[] = $No . '.';
              
                $dataRow[] = ($fetchRow[$key]['security_name'] == '' ? '-' : $fetchRow[$key]['security_name']);
    ...
    public function Div($id, $checked, $div){
        ...
        if($div == 'status'){
            ...
            $result  = "<button type='button' class='btn btn-warning btn-sm edit-security' data-id='$id' data-toggle='modal' data-target='#modal-security' id='edit-data' data-backdrop='static' data-keyboard='false' title='แก้ไขข้อมูล'>";
...
$DataTableCol = array( 
    0 => "tb_security.id_security",
    1 => "tb_security.id_security",
    2 => "tb_security.security_name",
    3 => "tb_security.security_status",
);
