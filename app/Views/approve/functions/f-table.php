    ...
    public function createArrayDataTable($fetchRow, $numRow){
        ...
        if (count($fetchRow) > 0) {
            ...
            foreach ($fetchRow as $key => $value) {
                ...
                $date    =   $this->getDateString($fetchRow[$key]['start_date'], $fetchRow[$key]['end_date']);
                $status  = ResStatusTable($fetchRow[$key]['reservation_status']);
                $control = $this->getControl($fetchRow[$key]['id_reservation']);
                $fetchRow[$key]['accessories'] == 1 ? $acc = $this->getAcc($fetchRow[$key]['id_reservation']) : $acc = "ไม่มี";
