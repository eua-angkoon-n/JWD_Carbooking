<script>
    $(document).ready(function () {
        ...
        $('input[name="res_status[]"], #res_vehicle, #res_date, #res_user').change(function () {
        ...
        // เมื่อคลิกที่ res_status1
    $('#res_status1').change(function() {
        // ถ้า res_status1 ถูกเลือก
        if ($(this).prop('checked')) {
            // เลือกทุก input ของ res_status และตั้งค่าให้ checked
            $('input[name="res_status[]"]').prop('checked', true);
        } else {
            // ถ้าไม่ใช่ ยกเลิกการเลือกทั้งหมด
            $('input[name="res_status[]"]').prop('checked', false);
        }
    });

    // เมื่อคลิกที่ res_status2 ถึง res_status8
    $('input[name="res_status[]"]').not('#res_status1').change(function() {
        // ถ้ามีปุ่มใดปุ่มหนึ่งถูก unchecked
        if (!$(this).prop('checked')) {
            // ยกเลิกการเลือกที่ res_status1
            $('#res_status1').prop('checked', false);
        } else {
            // ตรวจสอบว่าทุก input ของ res_status ถูกเลือกหรือไม่
            var allChecked = true;
            $('input[name="res_status[]"]').not('#res_status1').each(function() {
                if (!$(this).prop('checked')) {
                    allChecked = false;
                    return false; // ออกจากการวนลูป
                }
            });
            // ถ้าทุก input ของ res_status ถูกเลือก ให้เลือกที่ res_status1
            if (allChecked) {
                $('#res_status1').prop('checked', true);
            }
        }
    });
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    //     var button = '<div class="col-12 text-center"><button type="button" class="btn btn-primary backPage text-center w-75 h-100" id="backPage" title="กลับ">กลับ</span></button></div>';
                        $("#show_button").html('<div class="col-12 text-center"><button type="button" class="btn btn-primary backPage text-center w-75 h-100" id="backPage" title="กลับ">กลับ</span></button></div>');
                    // }
