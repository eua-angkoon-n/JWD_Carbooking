...
<div class="card-body p-3">
  <div class="row">
    <div class="col-sm-12 p-0 m-0">
      ...
      <table id="vehicle_type_table" class="table table-bordered table-hover dataTable dtr-inline table-responsive-xl">
        <thead>
          <tr class="bg-light">
...
<script type="text/javascript"> 
    ...
    $('#vehicle_type_table').DataTable({
      ...
      ajax: {
        ...
        url: 'app/Views/vehicleconfig/vehicle_type/f-table.php',
        type: 'POST',
        data: {
          "formData": 0,
          "action": "get"
    ...
    $(document).ready(function () {
      ...
      $('#vehicle_type_table_length').append('<div class="col-10 d-inline"><button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#modal-vehicle_type" id="addData" data-backdrop="static" data-keyboard="false"><i class="fas fa-plus-circle"></i> เพิ่มประเภทยานพาหนะ</button></div>');
      $('input[type=search]').attr('placeholder', 'ประเภทยานพาหนะ');

      $(document).off('click', '#addData').on('click', '#addData', function () {
        $("#id_vehicle_type").val('');
        $('#exampleModalLabel span').html("เพิ่มประเภทยานพาหนะ");
      });
    ...
    $(document).off('click', '.edit-vehicle_type').on('click', '.edit-vehicle_type', function () {
      $('#exampleModalLabel span').html("แก้ไขประเภทยานพาหนะ");
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (data) {
            ...
            $('#vehicle_type').val(jsonParse.vehicle_type);
            $('#id_vehicle_type').val(jsonParse.id_vehicle_type);

            $('#exampleModalLabel span').html("แก้ไขประเภทยานพาหนะ: " + jsonParse.vehicle_type);
            if (jsonParse.type_status == 1) {
              $('#status_use-vehicle_type').prop('checked', true);
              $('#status_hold-vehicle_type').prop('checked', false);
            } else {
              $('#status_use-vehicle_type').prop('checked', false);
              $('#status_hold-vehicle_type').prop('checked', true);
            }
          } else {
