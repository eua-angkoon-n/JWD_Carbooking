...
Class DataTable extends TableProcessing {
    ...
    public function getSQL(bool $OrderBY){
       if($OrderBY)
           ...
           $sql  = "SELECT count(id_vehicle_type) AS total_row ";
    ...
    public function createArrayDataTable($fetchRow, $numRow){
        ...
        if (count($fetchRow) > 0) {
            ...
            foreach ($fetchRow as $key => $value) {
                $id      = $fetchRow[$key]['id_vehicle_type'];
                $checked = ($fetchRow[$key]['type_status']==1 ? 'checked value="1" disabled' : ' disabled ');
    ...
    public function Div($id, $checked, $div){
        ...
        if($div == 'status'){
            ...
            $result  = "<button type='button' class='btn btn-warning btn-sm edit-vehicle_type' data-id='$id' data-toggle='modal' data-target='#modal-vehicle_type' id='edit-data' data-backdrop='static' data-keyboard='false' title='แก้ไขข้อมูล'>";
...
$DataTableCol = array( 
    0 => "tb_vehicle_type.id_vehicle_type",
    1 => "tb_vehicle_type.id_vehicle_type",
    2 => "tb_vehicle_type.vehicle_type",
    3 => "tb_vehicle_type.type_status",
);
