...
Class DataTable extends TableProcessing {
    ...
    public function getSQL(bool $OrderBY){
       if($OrderBY)
           ...
           $sql  = "SELECT count(id_expense) AS total_row ";
       $sql .= "FROM tb_expense ";
       $sql .= "WHERE 1=1 ";
       $sql .= "AND id_expense NOT IN (1)";
    ...
    public function createArrayDataTable($fetchRow, $numRow){
        ...
        if (count($fetchRow) > 0) {
            ...
            foreach ($fetchRow as $key => $value) {
                $id      = $fetchRow[$key]['id_expense'];
                $checked = ($fetchRow[$key]['expense_status']==1 ? 'checked value="1" disabled' : ' disabled ');

                $dataRow = array();
                $dataRow[] = $No . '.';
              
                $dataRow[] = ($fetchRow[$key]['expense_name'] == '' ? '-' : $fetchRow[$key]['expense_name']);
    ...
    public function Div($id, $checked, $div){
        ...
        if($div == 'status'){
            ...
            $result  = "<button type='button' class='btn btn-warning btn-sm edit-vehicle_expense' data-id='$id' data-toggle='modal' data-target='#modal-vehicle_expense' id='edit-data' data-backdrop='static' data-keyboard='false' title='แก้ไขข้อมูล'>";
...
$DataTableCol = array( 
    0 => "tb_expense.id_expense",
    1 => "tb_expense.id_expense",
    2 => "tb_expense.expense_name",
    3 => "tb_expense.expense_status",
);
