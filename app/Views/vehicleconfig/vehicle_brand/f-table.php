...
Class DataTable extends TableProcessing {
    ...
    public function getSQL(bool $OrderBY){
       if($OrderBY)
           ...
           $sql  = "SELECT count(id_vehicle_brand) AS total_row ";
    ...
    public function createArrayDataTable($fetchRow, $numRow){
        ...
        if (count($fetchRow) > 0) {
            ...
            foreach ($fetchRow as $key => $value) {
                $id      = $fetchRow[$key]['id_vehicle_brand'];
                $checked = ($fetchRow[$key]['brand_status']==1 ? 'checked value="1" disabled' : ' disabled ');
    ...
    public function Div($id, $checked, $div){
        ...
        if($div == 'status'){
            ...
            $result .= "<a class='dropdown-item edit-vehicle_brand' data-id='$id' data-toggle='modal' data-target='#modal-vehicle_brand' data-backdrop='static' data-keyboard='false' title='แก้ไขข้อมูล'>";
...
$DataTableCol = array( 
    0 => "tb_vehicle_brand.id_vehicle_brand",
    1 => "tb_vehicle_brand.id_vehicle_brand",
    2 => "tb_vehicle_brand.vehicle_brand",
    3 => "tb_vehicle_brand.brand_status",
);
